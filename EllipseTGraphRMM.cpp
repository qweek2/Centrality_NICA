...
ROOT::Math::Minimizer *ellipse_TGraph_minimize(TGraph *g)
{
...
#elif 1 /* 0 or 1 */
  ROOT::Math::Minimizer *m =
      ROOT::Math::Factory::CreateMinimizer("Minuit", "Combined");
#else   /* 0 or 1 */
  ROOT::Math::Minimizer *m =
      ROOT::Math::Factory::CreateMinimizer("Minuit", "Scan");
#endif  /* 0 or 1 */

  // set tolerance, etc. ...
  m->SetMaxFunctionCalls(1000000); // for Minuit
  ...
  if (theta < -45.0)
    theta += 180.0; // "theta" = -45 ... 135 degrees
...
void EllipseTGraphRMM(int run, TGraph *g = ((TGraph *)0))
{
  ...
  if (m && (!(m->Status())))
  {
    ...
    double top_x = xm[0] + xm[2] * cos(xm[4] * 3.1415 / 180);
    double top_y = xm[1] + xm[2] * sin(xm[4] * 3.1415 / 180);
